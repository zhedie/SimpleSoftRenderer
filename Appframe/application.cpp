...
void Application::init() {
    ...
    windowHandle = glfwCreateWindow(appSpec.width, appSpec.height, appSpec.name.c_str(), NULL, NULL);
...
Application::Application(const ApplicationSpecification &_appSpec) : appSpec(_appSpec) {
    instance = this;
    init();
}
    ...
    while (!glfwWindowShouldClose(windowHandle) && running) {
        ...
        if (g_SwapChainRebuild)
        {
            ...
            if (width > 0 && height > 0)
            {
                ...
                // Clear allocated command buffers since entire pool is destroyed
...
void Application::setMenubarCallback(const std::function<void()> &_menubarCallback) {
    menubarCallback = _menubarCallback;
}
